...
SQL_SCRIPTS = {
    ...
    "get_numeric_columns": {
        ...
        "postgres": """select server_name , table_catalog , table_schema , table_name , column_name from columns  WHERE SERVER_NAME = %s AND TABLE_CATALOG = %s AND TABLE_SCHEMA = %s AND TABLE_NAME = %s AND lower(DATA_TYPE) IN ('int', 'integer', 'decimal', 'numeric', 'float', 'money', 'tinyint', 'bigint', 'smallint', 'real');""",
        "sqlite3": """select server_name , table_catalog , table_schema , table_name , column_name from columns  WHERE SERVER_NAME = ? AND TABLE_CATALOG = ? AND TABLE_SCHEMA = ? AND TABLE_NAME = ? AND lower(DATA_TYPE) IN ('int', 'integer', 'decimal', 'numeric', 'float', 'money', 'tinyint', 'bigint', 'smallint', 'real');""",
        "mssqlserver": """select server_name , table_catalog , table_schema , table_name , column_name from columns  WHERE SERVER_NAME = ? AND TABLE_CATALOG = ? AND TABLE_SCHEMA = ? AND TABLE_NAME = ? AND lower(DATA_TYPE) IN ('int', 'integer', 'decimal', 'numeric', 'float', 'money', 'tinyint', 'bigint', 'smallint', 'real');""",
        "mariadb": """select server_name , table_catalog , table_schema , table_name , column_name from columns  WHERE SERVER_NAME = ? AND TABLE_CATALOG = ? AND TABLE_SCHEMA = ? AND TABLE_NAME = ? AND lower(DATA_TYPE) IN ('int', 'integer', 'decimal', 'numeric', 'float', 'money', 'tinyint', 'bigint', 'smallint', 'real');""",
    },
